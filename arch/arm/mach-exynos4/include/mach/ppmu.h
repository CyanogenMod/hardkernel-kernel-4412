/* linux/arch/arm/mach-exynos4/include/mach/ppmu.h
 *
 * Copyright (c) 2010 Samsung Electronics Co., Ltd.
 *		http://www.samsung.com/
 *
 * EXYNOS4 - PPMU support
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
*/

#ifndef __ASM_ARCH_PPMU_H
#define __ASM_ARCH_PPMU_H __FILE__

#define NUMBER_OF_COUNTER	4

#define DEVT0_SEL	0x1000
#define DEVT0_ID	0x1010
#define DEVT0_IDMSK	0x1014
#define DEVT1_ID	0x1110
#define DEVT1_IDMSK	0x1114
#define DEVT2_ID	0x1210
#define DEVT2_IDMSK	0x1214
#define DEVT3_ID	0x1310
#define DEVT3_IDMSK	0x1314

enum ppmu_counter {
	PPMU_PMNCNT0,
	PPMU_PMCCNT1,
	PPMU_PMNCNT2,
	PPMU_PMNCNT3,
	PPMU_PMNCNT_MAX,
};

enum ppmu_ch {
	DMC0,
	DMC1,
};

struct exynos4_ppmu_hw {
	void __iomem *hw_base;
	unsigned int ccnt;
	unsigned int event;
	unsigned int count[NUMBER_OF_COUNTER];
};

extern void exynos4_ppc_reset(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppc_start(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppc_stop(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppc_setevent(struct exynos4_ppmu_hw *ppmu,
				  unsigned int evt);
extern void exynos4_ppc_update(struct exynos4_ppmu_hw *ppmu);

extern void exynos4_ppmu_reset(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppmu_start(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppmu_stop(struct exynos4_ppmu_hw *ppmu);
extern void exynos4_ppmu_setevent(struct exynos4_ppmu_hw *ppmu,
		unsigned int evt, unsigned int evt_num);
extern void exynos4_ppmu_update(struct exynos4_ppmu_hw *ppmu);

/* mach-exynos4/busfreq.c */
struct notifier_block;
extern int register_busfreq_notifier(struct notifier_block *nb);
extern int unregister_busfreq_notifier(struct notifier_block *nb);
#endif /* __ASM_ARCH_PPMU_H */

