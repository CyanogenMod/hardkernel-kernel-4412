...
static int __init srp_probe(struct platform_device *pdev)
{
	...
	srp.iram	= ioremap(SRP_IRAM_BASE, 0x20000);
	srp.sram	= ioremap(SRP_SRAM_BASE, 0x40000);
	srp.clkcon	= ioremap(SRP_ASSCLK_BASE, 0x0010);
	srp.commbox	= ioremap(SRP_COMMBOX_BASE, 0x0200);

	/* Hidden register */
	srp.special	= ioremap(0x030F0000, 0x0100);

	if ((srp.iram == NULL) || (srp.sram == NULL) ||
		(srp.clkcon == NULL) || (srp.commbox == NULL) ||
		(srp.special == NULL)) {
		printk(KERN_ERR "SRP: ioremap error for Audio Subsystem\n");
